/*
This macro takes as input (for the event structure) the output of run_simHYDRA_laser.C
and sets the drift parameters, the magnetic field (by default the glad field with a 
current of 1500 A, though it can be changed) and the laser parameters.

Making use of the class R3BGTPCLaserGen the electron cloud generated by the passage of
the laser is simulated and saved as a set ProjPoints.
*/
...
void run_laser_gen(Bool_t constB = kFALSE)
{
    ...
    with a current of 1500 A (scale = current / 3583.81). However, it's possible to
    set a constant magnetic field running this macro as root -l 'run_laser_gen(kTRUE)'
    */
    ...
    lasergen->SetDriftParameters(15.e-9, 7E-03, 0.4522E-5, 0.1623E-5, 2);
