...
void R3BGTPCTrackFinder::eventToClusters(TClonesArray *hitCA,
                                         PointCloud &cloud) {
...
std::unique_ptr<R3BGTPCTrackData> R3BGTPCTrackFinder::clustersToTrack(
    PointCloud &cloud, const std::vector<cluster_t> &clusters,
    TClonesArray *trackCA, TClonesArray *hitCA) {
  ...
  for (auto &track : tracks) {
    ...
    new (clref[size])
        R3BGTPCTrackData(track.GetTrackId(), std::move(track.GetHitArray()),
                         std::move(*track.GetHitClusterArray()));
  }

  return NULL;
}

void R3BGTPCTrackFinder::Clusterize(R3BGTPCTrackData &track, Float_t distance,
                                    Float_t radius) {
  ...
  if (hitArray.size() > 0) {
    ...
    ROOT::Math::XYZVector refPos{hitArray.at(0).GetX(), hitArray.at(0).GetY(),
                                 hitArray.at(0).GetZ()};
    ...
    for (auto iHit = 0; iHit < hitArray.size(); ++iHit) {
      ...
      if (distRef < distance) {
        ...
        hitTBArray.clear();
        std::copy_if(hitArray.begin(), hitArray.end(),
                     std::back_inserter(hitTBArray),
                     [&refPos, radius](R3BGTPCHitData &hitIn) {
                       ROOT::Math::XYZVector hitInPos{
                           hitIn.GetX(), hitIn.GetY(), hitIn.GetZ()};
        ...
        if (hitTBArray.size() > 0) {
          ...
          int timeStamp = 0;
          std::shared_ptr<R3BGTPCHitClusterData> hitCluster =
              std::make_shared<R3BGTPCHitClusterData>();
          hitCluster->SetClusterID(clusterID);
          Double_t hitQ = 0.0;
          std::for_each(
              hitTBArray.begin(), hitTBArray.end(),
              [&x, &y, &z, &hitQ, &timeStamp](R3BGTPCHitData &hitInQ) {
                ROOT::Math::XYZPoint pos{hitInQ.GetX(), hitInQ.GetY(),
                                         hitInQ.GetZ()};
          ...
          for (auto iClusterHit : *track.GetHitClusterArray()) {
            ROOT::Math::XYZPoint iclusterHitPos{
                iClusterHit.GetX(), iClusterHit.GetY(), iClusterHit.GetZ()};
