/********************************************************************************
 * Copyright (C) 2024-2025 GSI Helmholtzzentrum fuer Schwerionenforschung GmbH *
...
R3BGTPCLaserGen::R3BGTPCLaserGen()
    : FairTask("R3BGTPCLaserGen"), fGTPCGeoPar(nullptr), fGTPCGasPar(nullptr),
      fGTPCElecPar(nullptr), fGTPCPointsCA(nullptr), fGTPCProjPointCA(nullptr),
      fGTPCCalDataCA(nullptr), MCTrackCA(nullptr) {
...
void R3BGTPCLaserGen::SetParameter() {
  ...
  fSizeOfVirtualPad =
      fGTPCGeoPar
          ->GetPadSize(); // 1 means pads of 1cm^2, 10 means pads of 1mm^2, ...
...
void R3BGTPCLaserGen::SetDriftParameters(Double_t ion, Double_t driftv,
...
void R3BGTPCLaserGen::SetLaserParameters(Double_t alpha, Double_t beta,
                                         Double_t x_in, Double_t y_in,
                                         Double_t z_in, Double_t maxLength) {
  fAlpha = alpha;         // deg
  fBeta = beta;           // deg
  fXIn = x_in;            // cm
  fYIn = y_in;            // cm
...
void R3BGTPCLaserGen::SetSizeOfVirtualPad(Double_t size) {
  fSizeOfVirtualPad =
      size; // 1 means pads of 1cm^2, 10 means pads of 1mm^2, ...
}

void R3BGTPCLaserGen::SetNumberOfGeneratedElectrons(Double_t ele) {
  fNumberOfGeneratedElectrons =
      ele; // Number of electrons to generate in each point of the test
}
...
void R3BGTPCLaserGen::Exec(Option_t *) {
  ...
  Double_t Vertex_x0 = 0, Vertex_y0 = 0, Vertex_z0 = 0, Vertex_px0 = 0,
           Vertex_py0 = 0, Vertex_pz0 = 0;
  ...
  Double_t mu =
      1.E+7 * fDriftVelocity /
      E_y; // [m2 s-1 V-1] TODO check value, move to parameter container
  ...
  Double_t TargetOffsetZ =
      fTargetOffsetZ; // USING INSTEAD THE FIELD MAP DESPLACEMENT! MISMATCH
  ...
  B_x = 0.1 * gladField->GetBx(
                  0, 0, 163.4); // Field components return in [kG], moved to [T]
  ...
  for (Int_t k = 0; k < fMaxLength; k++) {
    ...
    ele_z_init = TargetOffsetZ_FM - fHalfSizeTPC_Z -
                 sin(-TargetAngle) * (xval) + cos(-TargetAngle) * (zval);
    ...
    sigmaTransvAtPadPlane =
        sqrt(driftDistance * 2 * fTransDiff / fDriftVelocity);
    ...
    for (Int_t ele = 0; ele < fNumberOfGeneratedElectrons; ele++) {
      ...
      LOG(debug) << "R3BGTPCLaserGen::Exec, INITIAL VALUES: timeBeforeDrift="
                 << accDriftTime << " [ns]"
      ...
      while (ele_y > -fHalfSizeTPC_Y) // while not reaching the pad plane [cm]
      {
        ...
        vDrift_x = cteMult * (mu * (E_y * B_z) +
                              mu * mu * productEB *
                                  B_x); // cte * (Ex + mu*(E_y*B_z-E_z*B_y) +
                                        // mu*mu*productEB*B_x); SI
        vDrift_y =
            cteMult * (E_y + mu * mu * productEB *
                                 B_y); // cte * (Ey + mu*(E_z*B_x-E_x*B_z) +
                                       // mu*mu*productEB*B_y); SI
        vDrift_z = cteMult * (mu * (-E_y * B_x) +
                              mu * mu * productEB *
                                  B_z); // cte * (Ez + mu*(E_x*B_y-E_y*B_x) +
                                        // mu*mu*productEB*B_z); SI

        LOG(debug) << "R3BGTPCLaserGen::Exec, timeBeforeDrift=vDrift_x="
                   << vDrift_x << " vDrift_y=" << vDrift_y
                   << " vDrift_z=" << vDrift_z << " [m/s]";

        // adjusting the last step before the pad plane
        if (ele_y - 1.E-7 * vDrift_y * driftTimeStep < -fHalfSizeTPC_Y)
          driftTimeStep = (ele_y + fHalfSizeTPC_Y) /
                          (1.E-7 * vDrift_y); // vDrift back to [cm/ns]
        ...
        sigmaTransvStep =
            sqrt(driftTimeStep * 2 * fTransDiff *
                 cteMod); // should be reduced by the factor cteMod=cteMult/mu
        sigmaLongStep = sqrt(
            driftTimeStep * 2 *
            fLongDiff); // should be the same scaled to the length of the step
        ele_x = gRandom->Gaus(ele_x + 1.E-7 * vDrift_x * driftTimeStep,
                              sigmaTransvStep); // vDrift back to [cm/ns]
        ele_y = gRandom->Gaus(ele_y - 1.E-7 * vDrift_y * driftTimeStep,
                              sigmaLongStep);
        ele_z = gRandom->Gaus(ele_z + 1.E-7 * vDrift_z * driftTimeStep,
                              sigmaTransvStep);
        accDriftTime = accDriftTime + driftTimeStep;

        LOG(debug) << "R3BGTPCLaserGen::Exec, accDriftTime=" << accDriftTime
                   << " [ns]"
                   << " ele_x=" << ele_x << " ele_y=" << ele_y
                   << " ele_z=" << ele_z << " [cm]";
      }
      ...
      projX = +cos(TargetAngle) * ele_x +
              sin(TargetAngle) * (ele_z - (TargetOffsetZ_FM - fHalfSizeTPC_Z));
      projZ = -sin(TargetAngle) * ele_x +
              cos(TargetAngle) * (ele_z - (TargetOffsetZ_FM - fHalfSizeTPC_Z));

      // Removing electrons out of pad plane limits
      if (projZ < fOffsetZ || projZ > fOffsetZ + 2 * fHalfSizeTPC_Z ||
          projX < fOffsetX || projX > fOffsetX + 2 * fHalfSizeTPC_X)
        continue;
      ...
      if (outputMode == 0) { // Output: TClonesArray of R3BGTPCCalData
        ...
        for (Int_t pp = 0; pp < nProjPoints; pp++) {
          if (((R3BGTPCProjPoint *)fGTPCProjPointCA->At(pp))
                  ->GetVirtualPadID() == padID) {
        ...
        if (!padFound) {
          new ((*fGTPCProjPointCA)[nProjPoints]) R3BGTPCProjPoint(
              padID,
              projTime / fTimeBinSize, // micros
              1, evtID, PDGCode, MotherId, Vertex_x0, Vertex_y0, Vertex_z0,
              Vertex_px0, Vertex_py0, Vertex_pz0);
        }
        padFound = kFALSE;
      }
    }
