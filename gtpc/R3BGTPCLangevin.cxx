...
R3BGTPCLangevin::R3BGTPCLangevin()
    : FairTask("R3BGTPCLangevin"), fGTPCPointsCA(NULL), fGTPCCalDataCA(NULL),
      fGTPCProjPointCA(NULL), fMCTrackCA(NULL) {
  fEIonization = 0;
  fDriftVelocity = 0;
  fTransDiff = 0;
  fLongDiff = 0;
...
void R3BGTPCLangevin::SetParameter() {
  ...
  fDriftVelocity =
      fGTPCGasPar
          ->GetDriftVelocity(); // [cm/ns]-> Minos TPC with a Efield=152 V/cm
  ...
  fDriftTimeStep =
      fGTPCElecPar
          ->GetDriftTimeStep(); // time step for drift params calculation
  fTimeBinSize =
      fGTPCElecPar->GetTimeBinSize(); // time step for drift params calculation
}
...
void R3BGTPCLangevin::Exec(Option_t *) {
  ...
  for (Int_t i = 0; i < nPoints; i++) {
    ...
    } else { // any other case, that is, other than entering the volume
      if (presentTrackID !=
          aPoint->GetTrackID()) { // track was not entering the volume in a
                                  // previous point, what somehow it is in! :‑O
        LOG(fatal) << "R3BGTPCLangevin::Exec: Problem 2 in point logic";
        break;
      }
      if (readyToProject != kTRUE) { // track somehow exited the gas volume or
                                     // dissappeared in a previous point :‑O
        LOG(fatal) << "R3BGTPCLangevin::Exec: Problem 3 in point logic";
        break;
      }
      if (aPoint->GetTrackStatus() == 10100 ||
          aPoint->GetTrackStatus() ==
              1000000) { // exiting the gas volume or dissappearing
    ...
    generatedElectrons =
        gRandom->Gaus(electrons, flucElectrons); // generated electrons
    ...
    for (Int_t ele = 1; ele <= generatedElectrons; ele++) {
      ...
      ele_x = xPre +
              stepX * ele; // homogeneous electron creation along the step [cm]
      ele_y = yPre + stepY * ele;
      ele_z = zPre + stepZ * ele;
      accDriftTime = timeBeforeDrift;

      LOG(debug) << "R3BGTPCLangevin::Exec, INITIAL VALUES: timeBeforeDrift="
                 << accDriftTime << " [ns]"
      ...
      while (ele_y > -fHalfSizeTPC_Y) { // while not reaching the pad plane [cm]
        B_x = 1e4 * gladField->GetBx(ele_x, ele_y,
        ...
        moduleB =
            TMath::Sqrt(B_x * B_x + B_y * B_y + B_z * B_z); // in [V ns cm^-2]
        ...
        LOG(debug) << "R3BGTPCLangevin::Exec, DRIFT VELOCITIES: vDrift_x="
        ...
        sigmaTransvStep =
            sqrt(fDriftTimeStep * 2 * fTransDiff *
        ...
        LOG(debug) << "R3BGTPCLangevin::Exec, NEW VALUES: accDriftTime="
                   << accDriftTime << " [ns]"
                   << " ele_x=" << ele_x << " ele_y=" << ele_y
                   << " ele_z=" << ele_z << " [cm]";
      }
  ...
  if (outputMode == 0)
    LOG(info) << "R3BGTPCLangevin: produced " << fGTPCCalDataCA->GetEntries()
              << " R3BGTPCcalData(s)";
  if (outputMode == 1)
    LOG(info) << "R3BGTPCLangevin: produced " << fGTPCProjPointCA->GetEntries()
              << " R3BGTPCProjPoint(s)";
}
