...
R3BGTPCCal2Hit::R3BGTPCCal2Hit()
    : PadCoordArr(boost::extents[5632][4][2]), FairTask("R3B GTPC Cal to Hit"),
      fCalCA(NULL), fHitCA(NULL), fTPCMap(NULL), fOnline(kFALSE),
      fLangevinBack(kTRUE) {
...
void R3BGTPCCal2Hit::Exec(Option_t *opt) {
  ...
  R3BGladFieldMap *gladField =
      (R3BGladFieldMap *)FairRunAna::Instance()->GetField(); // B Field
  if (!gladField) {
  ...
  for (Int_t i = 0; i < nCals; i++) {
    ...
    for (auto iadc = 0; iadc < adc_cal.size(); iadc++) {
      ...
      z = (TargetOffsetZ_FM - fHalfSizeTPC_Z) - sin(-TargetAngle) * (xold) +
          cos(-TargetAngle) * (zold);

      time =
          time * fTimeBinSize +
          0.5 *
              fTimeBinSize; //[ns] moving from TimeBuckets to ns; adding the
                            // half of
      ...
      if (fLangevinBack == kTRUE) {
        ...
        LOG(debug) << "R3BGTPCCal2Hit::Exec, INITIAL VALUES: \tTimeToRun="
                   << accDriftTime << " [ns]"
                   << " \tx=" << x << "  \ty=" << y << " \tz=" << z << " [cm]";
        ...
        while (accDriftTime > 0.) {
          ...
          moduleB =
              TMath::Sqrt(B_x * B_x + B_y * B_y + B_z * B_z); // [V ns cm^-2]
          ...
          vDrift_x = cteMult *
                     (mu * (E_y * B_z) + mu * mu * productEB * B_x); //[cm/ns]
          vDrift_y = cteMult * (E_y + mu * mu * productEB * B_y);    //[cm/ns]
          vDrift_z = cteMult *
                     (mu * (-E_y * B_x) + mu * mu * productEB * B_z); //[cm/ns]
          ...
          LOG(debug) << "R3BGTPCCal2Hit::Exec, NEW VALUES: accDriftTime="
                     << accDriftTime << " [ns]"
                     << " x=" << x << " y=" << y << " z=" << z << " [cm]"
                     << " Drift_v " << vDrift_x << " fDriftTimeStep "
                     << fDriftTimeStep;
        }
        ...
        LOG(debug) << "Comparing sigmas... Approx: " << sigmaLong << " "
                   << sigmaTransv
                   << ";  Step by step: " << TMath::Sqrt(cloudLong) << " "
                   << TMath::Sqrt(cloudTransv);
      }
      ...
      x = +cos(TargetAngle) * xnew +
          sin(TargetAngle) * (znew - (TargetOffsetZ_FM - fHalfSizeTPC_Z));
      z = -sin(TargetAngle) * xnew +
          cos(TargetAngle) * (znew - (TargetOffsetZ_FM - fHalfSizeTPC_Z));
...
R3BGTPCHitData *R3BGTPCCal2Hit::AddHitData(Double_t x, Double_t y, Double_t z,
                                           Double_t longWidth,
                                           Double_t energy) {
