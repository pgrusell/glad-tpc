...
R3BGTPCProjector::R3BGTPCProjector()
    : FairTask("R3BGTPCProjector"), fGTPCPoints(NULL), fGTPCCalDataCA(NULL),
      fGTPCProjPoint(NULL), MCTrackCA(NULL)
...
void R3BGTPCProjector::Exec(Option_t *) {
  ...
  for (Int_t i = 0; i < nPoints; i++) {
    ...
    for (Int_t ele = 1; ele <= generatedElectrons;
         ele++) // following each electrons from production to pad
    {
      ...
      projTime = gRandom->Gaus(driftTime + timeBeforeDrift,
                               sigmaLongAtPadPlane / fDriftVelocity);
  ...
  */
      ...
      if (outputMode == 0) { // Output: TClonesArray of R3BGTPCCalData
        ...
        for (Int_t pp = 0; pp < nProjPoints; pp++) {
          if (((R3BGTPCProjPoint *)fGTPCProjPoint->At(pp))->GetVirtualPadID() ==
              padID) {
            ...
            ((R3BGTPCProjPoint *)fGTPCProjPoint->At(pp))
                ->SetTimeDistr(projTime / 1000, 1); // micros
            padFound = kTRUE;
            break;
          }
        }
        if (!padFound) {
          new ((*fGTPCProjPoint)[nProjPoints]) R3BGTPCProjPoint(
              padID,
  ...
  LOG(info) << "R3BGTPCProjector: produced " << fGTPCProjPoint->GetEntries()
            << " projPoints";
}
