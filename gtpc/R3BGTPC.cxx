...
R3BGTPC::R3BGTPC(const TString &geoFile, const TGeoTranslation &trans,
...
void R3BGTPC::Initialize() {
  ...
  LOG(info) << "R3BGTPC: Active_region Vol. (McId) "
            << gMC->VolId("Active_region");
}
...
void R3BGTPC::SetSpecialPhysicsCuts() {
  ...
  if (gGeoManager) { // NOT SURE THIS IS USEFUL
    ...
    if (pmix) {
      ...
      LOG(info) << "-I- R3BGTPC: Medium Id " << pmix->GetId()
                << " Energy Cut-Off : " << cutE << " GeV";
      ...
      gMC->Gstpar(pmix->GetId(), "BCUTE",
                  cutE); /** electron bremsstrahlung (GeV)*/
      gMC->Gstpar(pmix->GetId(), "BCUTM",
                  cutE); /** muon and hadron bremsstrahlung(GeV)*/
      gMC->Gstpar(pmix->GetId(), "DCUTE",
                  cutE); /** delta-rays by electrons (GeV)*/
      gMC->Gstpar(pmix->GetId(), "DCUTM", cutE); /** delta-rays by muons (GeV)*/
      gMC->Gstpar(pmix->GetId(), "PPCUTM",
                  -1.); /** direct pair production by muons (GeV)*/
    }
  } //! gGeoManager
}
...
Bool_t R3BGTPC::ProcessHits(FairVolume *vol) {
  ...
  Int_t theTrackStatus = GetTrackStatus(
      gMC->IsNewTrack(), gMC->IsTrackDisappeared(), gMC->IsTrackStop(),
      gMC->IsTrackAlive(), gMC->IsTrackEntering(), gMC->IsTrackExiting(),
      gMC->IsTrackInside(), gMC->IsTrackOut());
  ...
  if (gMC->TrackPid() != 0) // due to the INCL generator
  {
    ...
    new ((*fGTPCPointCollection)[size]) R3BGTPCPoint(
...
Int_t R3BGTPC::GetTrackStatus(bool NewTrack, bool TrackDisappeared,
                              bool TrackStop, bool TrackAlive,
                              bool TrackEntering, bool TrackExiting,
                              bool TrackInside, bool TrackOut) {
  int trackstatus = 1 * TrackOut + 10 * TrackInside + 100 * TrackExiting +
                    1000 * TrackEntering + 10000 * TrackAlive +
                    100000 * TrackStop + 1000000 * TrackDisappeared +
                    10000000 * NewTrack;
