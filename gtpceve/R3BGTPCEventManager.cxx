...
R3BGTPCEventManager::R3BGTPCEventManager()
    : TEveEventManager("R3BGTPCEventManager", ""),
      ...
      fRunAna(FairRunAna::Instance()), fEntry(0), fEvent(0), fCurrentEvent(0),
      fCvsPadPlane(0), fPadWave(0)
...
void R3BGTPCEventManager::Init(Int_t option, Int_t level, Int_t nNodes) {
  ...
  gVirtualX->GetWindowSize(gClient->GetRoot()->GetId(), dummy, dummy, width,
                           height);
...
void R3BGTPCEventManager::GotoEvent(Int_t event) {
  ...
  fEntry = event;
  std::cout << cWHITERED << " Event number : " << fEntry << cNORMAL
            << std::endl;
...
void R3BGTPCEventManager::make_gui() {
  ...
  fCurrentEvent =
      new TGNumberEntry(f, 0., 6, -1, TGNumberFormat::kNESInteger,
                        TGNumberFormat::kNEANonNegative,
                        TGNumberFormat::kNELLimitMinMax, 0, Entries);
  f->AddFrame(fCurrentEvent, new TGLayoutHints(kLHintsLeft, 1, 1, 1, 1));
  fCurrentEvent->Connect("ValueSet(Long_t)", "R3BGTPCEventManager", fInstance,
                         "SelectEvent()");
  frmMain->AddFrame(f);
