...
R3BGTPCEventDrawTask::R3BGTPCEventDrawTask()
    : fCvsPadPlane(0), fPadPlane(0), fMap(0), fHitSet(0), fCvsPadWave(0),
      fPadWave(0), fHitCA(0), fTrackCA(0) {}
...
InitStatus R3BGTPCEventDrawTask::Init() {
  ...
  if (fHitCA)
    LOG(info) << cGREEN << "Hit Array Found in branch GTPCHitData." << cNORMAL
              << std::endl;

  fTrackCA = (TClonesArray *)ioMan->GetObject("GTPCTrackData");
  if (fTrackCA)
    LOG(info) << cGREEN << "Track Array Found in branch GTPCTrackData."
              << cNORMAL << std::endl;
...
void R3BGTPCEventDrawTask::DrawTracks() {
  ...
  for (auto i = 0; i < nTracks; ++i) {
    ...
    fTrackHitSet.push_back(new TEvePointSet(
        Form("TrackHit_%d", i), 0, TEvePointSelectorConsumer::kTVT_XYZ));
    ...
    std::vector<R3BGTPCHitClusterData> *trackClusterHits =
        trackData[i]->GetHitClusterArray();
    ...
    if (trackClusterHits->size() > 0) {
      ...
      for (auto trackHitCluster : *trackClusterHits) {
        ...
        fHitClusterSet[i]->AddBox(xC - boxSize / 2.0, yC - boxSize / 2.0,
                                  zC - boxSize / 2.0, boxSize, boxSize,
                                  boxSize);
      }
...
EColor R3BGTPCEventDrawTask::GetTrackColor(int i) {
  std::vector<EColor> colors = {kAzure, kOrange, kViolet, kTeal, kMagenta,
                                kBlue,  kViolet, kYellow, kCyan, kAzure};
  if (i < 10) {
    return colors.at(i);
  } else
    return kAzure;
