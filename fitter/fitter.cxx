...
int main(int argc, char *argv[]) {
  ...
  TH2F *XYVertex =
      new TH2F("XYVertex", "XYVertex", 1000, -25, 25, 1000, -25, 25);
  TH2F *XZVertex =
      new TH2F("XZVertex", "XZVertex", 1000, -25, 25, 1000, 200, 300);
  ...
  genfit::FieldManager::getInstance()->init(
      new genfit::ConstField(0.0, 20.0, 0.0)); //
  ...
  fTargetPlane =
      genfit::SharedPlanePtr(new genfit::DetPlane(posTargetIni, normalTarget));
    ...
    for (auto iTrack = 0; iTrack < nTracks; ++iTrack) {
      ...
      try {
        ...
        if (fitTrack && fitTrack->hasKalmanFitStatus()) {
          ...
          auto trackRep = fitTrack->getTrackRep(
              0); // Only one representation is sved for the moment.
          ...
          if (KalmanFitStatus->isFitConverged()) {
            ...
            Double_t E = TMath::Sqrt(TMath::Power(mom_res.Mag(), 2) +
                                     TMath::Power(M_Ener, 2)) -
                         M_Ener;
            energy->Fill(E);
...
HelixTrackModel::HelixTrackModel(const TVector3 &pos, const TVector3 &mom,
                                 double charge) {
...
TVector3 HelixTrackModel::getPos(double tracklength) const {
  ...
  pos = center_ + radius;
  pos.SetZ(center_.Z() -
           sgn_ * ((alpha0_ - angle) * R_ * tan(theta_ - M_PI / 2.)));

  return pos;
}

void HelixTrackModel::getPosMom(double tracklength, TVector3 &pos,
                                TVector3 &mom) const {
